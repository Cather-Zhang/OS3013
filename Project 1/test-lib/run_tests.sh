...
print_error_message () {
    ...
    echo "  expected results here: $testdir/$testnum.$filetype"
    echo "  results produced by your program here: tests-out/$testnum.$filetype"
    echo "  compare the two files using diff, cmp, or related tools to debug, e.g.:"
    echo "  prompt> diff $testdir/$testnum.$filetype tests-out/$testnum.$filetype"
    echo "  it might also help to view: tests-out/$testnum.err"
